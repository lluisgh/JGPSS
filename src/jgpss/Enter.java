...
public class Enter extends Bloc {
    ...
    public Bloc execute(Xact tr) throws Exception {
        ...
        System.out.println("Instàncies que es volen capturar: " + B);
        if (getModel().getStorages().containsKey(A)) {              //Si existeix l'Storage A
            ...
            if (s.getLliures() >= B) {                              //Si hi ha prou instàncies del servidor A disponibles
                System.out.println("Hi ha prou instàncies lliures (" + s.getLliures() + ").");
                ...
                if (o.containsKey(tr.getID())) o.put(tr.getID(), o.get(tr.getID() + B)); //Si la transacció actual es troba al map, incrementem en B                                                                  //el nombre d'instàncies del servidor ocupades per aquesta
                ...
                System.out.println("No hi ha prou instàncies lliures.");
